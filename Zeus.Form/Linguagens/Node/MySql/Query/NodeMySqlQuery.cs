...
namespace Zeus.Linguagens.Node.MySql.Query
{
    public class NodeMySqlQuery : BaseMySqlDAO
    {
        ...
        private StringBuilder GetById()
        {
            ...
            get.Append($"                return mysql.executeString(`SELECT * FROM {NomeTabela.ToLower()} WHERE {ListaAtributosTabela.First().COLUMN_NAME} = ${{id}}`);{N}");
            get.Append($"        }},{N}");
            return get;
        }
        ...
        private StringBuilder Delete()
        {
            ...
            get.Append($"                return mysql.executeString(`DELETE FROM {NomeTabela.ToLower()} WHERE {ListaAtributosTabela.First().COLUMN_NAME} = ${{id}}`);{N}");
            get.Append($"        }},{N}");
            return get;
        }
