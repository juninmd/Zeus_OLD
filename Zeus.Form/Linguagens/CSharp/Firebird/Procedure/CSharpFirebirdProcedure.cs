    ...
    public class CSharpFirebirdProcedure : BaseFirebirdDAO
    {
        ...
        private StringBuilder GetListaItensGetById()
        {
            ...
            foreach (var item in ListaAtributosTabela)
                atributoText.Append(
                    $"                        {item.FIELD_NAME} = \"{item.FIELD_NAME}\".GetValueOrDefault<{CSharpTypesFirebird.GetTypeAtribute(item.FIELD_NAME, item.IS_NULLABLE)}>(reader)," +
                    N);
            return atributoText;
        }
        ...
        private StringBuilder GetListaItensAdd()
        {
            var atributoText = new StringBuilder();
            foreach (var item in ListaAtributosTabela)
                atributoText.Append($"            AddParameter(\"{item.FIELD_NAME}\", entidade.{item.FIELD_NAME});" +
                                    N);
            return atributoText;
        }
