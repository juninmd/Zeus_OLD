    ...
    public class JavaPostgreEntidade : BasePostgreDAO
    {
        ...
        private StringBuilder AtributosBody(List<PostgreEntidadeTabela> entidadeTabela)
        {
            ...
            foreach (var att in entidadeTabela)
            {
                ...
                atributoBody.Append($"	 */{N}");
                atributoBody.Append(
                    $"	public {JavaTypesPostgre.GetTypeAtribute(att)} get{att.COLUMN_NAME.ToFirstCharToUpper()}() {{{N}");
                atributoBody.Append($"		return {att.COLUMN_NAME};{N}");
                atributoBody.Append($"	}}{N}");
                atributoBody.Append($"{N}");

                atributoBody.Append(
                    $"	public void set{att.COLUMN_NAME.ToFirstCharToUpper()}({JavaTypesPostgre.GetTypeAtribute(att)} {att.COLUMN_NAME}) {{{N}");
