    ...
    public class JavaPostgreQuery : BasePostgreDAO
    {
        ...
        private StringBuilder GetById()
        {
            ...
            foreach (var att in ListaAtributosTabela)
                get.Append(
                    $"				resposta.set{att.COLUMN_NAME.ToFirstCharToUpper()}(rs.get{JavaTypesPostgre.GetTypeAtribute(att).ToFirstCharToUpper()}(\"{att.COLUMN_NAME}\"));{N}");
        ...
        private StringBuilder Add()
        {
            ...
            for (var index = 1; index < ListaAtributosTabela.Count; index++)
            {
                ...
                get.Append(
                    $"			conn.set{JavaTypesPostgre.GetTypeAtribute(att).ToFirstCharToUpper()}({index}, entidade.get{att.COLUMN_NAME.ToFirstCharToUpper()}());{N}");
            }
