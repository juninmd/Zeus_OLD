...
namespace Zeus.Linguagens.Java.MySql.Query
{
    public class JavaMySqlQuery : BaseMySqlDAO
    {
        public JavaMySqlQuery(string nomeTabela) : base(nomeTabela)
        {
        }
        ...
        private StringBuilder GetById()
        {
            ...
            foreach (var att in ListaAtributosTabela)
                get.Append(
                    $"				resposta.set{att.COLUMN_NAME.ToFirstCharToUpper()}(rs.get{JavaTypesMySql.GetTypeAtribute(att).ToFirstCharToUpper()}(\"{att.COLUMN_NAME}\"));{N}");
        ...
        private StringBuilder Add()
        {
            ...
            for (var index = 1; index < ListaAtributosTabela.Count; index++)
            {
                ...
                get.Append(
                    $"			conn.set{JavaTypesMySql.GetTypeAtribute(att).ToFirstCharToUpper()}({index}, entidade.get{att.COLUMN_NAME.ToFirstCharToUpper()}());{N}");
            }
