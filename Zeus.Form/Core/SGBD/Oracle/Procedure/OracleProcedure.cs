...
namespace Zeus.Core.SGBD.Oracle.Procedure
{
    public class OracleProcedure : BaseOracleDAO
    {
        ...
        private StringBuilder MontaTodosParametrosHeader(string nomeProcedure, bool isCursor, bool isHeader,
            bool onlyFirst = false, bool semparametros = false, bool ocultaPrimeiro = false)
        {
            var parametro = new StringBuilder();
            parametro.Append($"  PROCEDURE {nomeProcedure}(");


            parametro.Append(isCursor
                ? "P_CURSORSELECT               OUT TP_CURSOR"
                : "P_RESULT               OUT STRING");
                ...
                if (onlyFirst == false)
                    for (var i = ocultaPrimeiro ? 1 : 0; i < ListaAtributosTabela.Count; i++)
                        if (i == ListaAtributosTabela.Count - 1)
                            parametro.Append(EspacoMontadoParametros(nomeProcedure) +
                                             $"P_{ListaAtributosTabela[i].COLUMN_NAME}        IN {NomeTabela}.{ListaAtributosTabela[i].COLUMN_NAME}%TYPE");
                        else
                            parametro.Append(EspacoMontadoParametros(nomeProcedure) +
                                             $"P_{ListaAtributosTabela[i].COLUMN_NAME}        IN {NomeTabela}.{ListaAtributosTabela[i].COLUMN_NAME}%TYPE, " +
                                             Environment.NewLine);
                    ...
                    parametro.Append(EspacoMontadoParametros(nomeProcedure) +
                                     $"P_{ListaAtributosTabela[0].COLUMN_NAME}        IN {NomeTabela}.{ListaAtributosTabela[0].COLUMN_NAME}%TYPE");
            }
        ...
        private StringBuilder MontaTodosParametrosUpdate()
        {
            ...
            for (var i = 0; i < ListaAtributosTabela.Count; i++)
                if (i == ListaAtributosTabela.Count - 1)
                    parametro.Append(
                        $"             {ListaAtributosTabela[i].COLUMN_NAME}        = P_{ListaAtributosTabela[i].COLUMN_NAME}" +
                        Environment.NewLine);
                else if (i == 0)
                    parametro.Append(
                        $"{ListaAtributosTabela[i].COLUMN_NAME}        = P_{ListaAtributosTabela[i].COLUMN_NAME}," +
                    ...
                    parametro.Append(
                        ...
                        Environment.NewLine);
            parametro.Append(
                $"         WHERE {ListaAtributosTabela[0].COLUMN_NAME}        = P_{ListaAtributosTabela[0].COLUMN_NAME};");
